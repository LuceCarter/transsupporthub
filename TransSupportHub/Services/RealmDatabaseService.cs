...
namespace TransSupportHub
{
    public static class RealmService
    {
        ...
        private static (IQueryable<Clinician> Query, string Name) GetQueryForSubscriptionType(Realm realm, SubscriptionType subType)
        {
            IQueryable<Clinician> query = null;
            string queryName = null;

            if (subType == SubscriptionType.Mine)
            {
                query = realm.All<Clinician>().Where(i => i.OwnerId == CurrentUser.Id);
                queryName = "mine";
            }
            else if (subType == SubscriptionType.All)
            {
                query = realm.All<Clinician>();
                queryName = "all";
            }
            else
            {
